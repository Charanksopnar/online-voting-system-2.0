...
import { Play, Clock, StopCircle, RefreshCw, Trash2, Plus, MapPin } from 'lucide-react';
...
export const AddElection = () => {
    ...
    const districtsForState = useMemo(() => {
        return form.regionState ? INDIAN_STATES_DISTRICTS[form.regionState] || [] : [];
    }, [form.regionState]);
    ...
    const handleRegionChange = (region: string) => {
        setForm({
            ...form,
            region,
            regionState: region === 'National' ? '' : form.regionState,
            regionDistrict: region !== 'District' ? '' : form.regionDistrict
        });
    };
    ...
    const handleCreate = (e: React.FormEvent) => {
        ...
        // Validation for non-national elections
        if (form.region !== 'National' && !form.regionState) {
            addNotification('ERROR', 'Required', 'Please select a state');
            return;
        }
        if (form.region === 'District' && !form.regionDistrict) {
            addNotification('ERROR', 'Required', 'Please select a district');
            return;
        }
        ...
        addElection({
            ...
            status: 'UPCOMING',
            region: form.region,
            regionState: form.regionState || undefined,
            regionDistrict: form.regionDistrict || undefined,
            description: '',
            candidates: []
        });
        setShowAddForm(false);
        setForm({ title: '', start: '', end: '', region: 'National', regionState: '', regionDistrict: '' });
        addNotification('SUCCESS', 'Created', 'Election Scheduled');
    };

    const getLocationDisplay = (ele: typeof elections[0]) => {
        if (ele.region === 'National') return 'National Level';
        if (ele.region === 'State' && ele.regionState) return `${ele.regionState} (State)`;
        if (ele.region === 'District' && ele.regionDistrict) return `${ele.regionDistrict}, ${ele.regionState} (District)`;
        return `${ele.region} Level`;
    };
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 rounded-2xl mb-8 shadow-md">
                    ...
                    <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select className="input-standard" value={form.region} onChange={e => handleRegionChange(e.target.value)}>
                        ...
                        {/* State Dropdown - Show for State and District level elections */}
                        {form.region !== 'National' && (
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e => setForm({ ...form, regionState: e.target.value, regionDistrict: '' })}
                                >
                        ...
                        {/* District Dropdown - Show only for District level elections */}
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700 text-sm">
                            ...
                            <tr key={ele.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition">
                                ...
                                <td className="p-4">
                                    <div className="flex items-center gap-2 text-sm">
                                        ...
                                        <span className="text-slate-600 dark:text-slate-300">{getLocationDisplay(ele)}</span>
                                    </div>
                                </td>
                                <td className="p-4">
                                ...
                                <td className="p-4">
                                    <span className={`px-3 py-1 rounded-full font-bold text-[10px] uppercase tracking-wider ${ele.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
                        ...
                        {elections.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-slate-500">No elections found.</td></tr>}
                    </tbody>
                </table>
            </div>
        </div>
