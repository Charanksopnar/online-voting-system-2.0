...
import { Calendar, MapPin, Shield, Award, AlertCircle, Fingerprint, Clock, CheckCircle, AlertTriangle } from 'lucide-react';

export const UserDashboard = () => {
  const { user } = useAuth();
  const { elections, votes, candidates } = useRealtime();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-8">
                ...
                {/* Verification Alert */}
                {!isVerified && (
                    <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 p-5 rounded-xl flex gap-4 animate-pulse">
                        <AlertCircle className="text-yellow-600 dark:text-yellow-500 flex-shrink-0" size={24} />
                        <div>
                            <h4 className="font-bold text-yellow-800 dark:text-yellow-400">Identity Unverified</h4>
                            <p className="text-sm text-yellow-700 dark:text-yellow-500 mt-1">
                                You cannot vote until your biometrics are approved.
                            </p>
                            <button onClick={handleKycClick} className="text-sm font-bold text-yellow-900 dark:text-yellow-300 underline mt-2 inline-block">Complete KYC Now &rarr;</button>
                        </div>
                    </div>
                )}
            </div>
                                                ...
                                                {election.status === 'ACTIVE' ? (
                                                    hasVoted ? (
                                                        ...
                                                        <button 
                                                            ...
                                                            className={`px-6 py-2.5 font-bold rounded-lg shadow-lg transition transform hover:-translate-y-0.5 flex items-center gap-2 ${
                                                                isVerified 
